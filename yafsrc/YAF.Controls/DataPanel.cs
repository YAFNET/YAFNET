namespace YAF.Controls
{
  ...
  public class DataPanel : Panel, IPostBackDataHandler
  {
    ...
    {
      ScriptManager.RegisterClientScriptInclude(this, GetType(), _dataPanelScript, YafForumInfo.GetURLToResource("js/DataPanel.js"));
    }
    ...
    /// </remarks>
    [Description("Set to true to allow clicking the panel" + "header title to expand/collapse the content panel."), Category("Appearance"), DefaultValue(false)]
    public bool AllowTitleExpandCollapse
    {
      get
      {
        if (ViewState[_allowTitleExpandCollapse] == null)
        {
          return false;
        }

        return Convert.ToBoolean(ViewState[_allowTitleExpandCollapse]);
      }

      set
      {
        ViewState[_allowTitleExpandCollapse] = value;
      }
    }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Set to true to collapse the content panel," + " else false to expand."), Category("Appearance"), DefaultValue(true)]
    public bool Collapsed
    {
      get
      {
        if (ViewState[_collapsed] == null)
        {
          return true;
        }

        return Convert.ToBoolean(ViewState[_collapsed]);
      }

      set
      {
        ViewState[_collapsed] = value;
      }
    }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Set to true to enable the content panel to" + " collapse, else false to disable."), Category("Appearance"), DefaultValue(true)]
    public bool Collapsible
    {
      get
      {
        if (ViewState[_collapsible] == null)
        {
          return true;
        }

        return Convert.ToBoolean(ViewState[_collapsible]);
      }

      set
      {
        ViewState[_collapsible] = value;
      }
    }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Url of the image to use for the collapse-" + "action link."), Category("Appearance"), DefaultValue("")]
    public string CollapseImageUrl
    {
      get
      {
        ...
        return ViewState[_collapseImageUrl].ToString();
      }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Text to use for the collapse-action link."), Category("Appearance"), DefaultValue("Collapse")]
    public string CollapseText
    {
      get
      {
        if (ViewState[_collapseText] == null)
        {
          return "Collapse";
        }

        return ViewState[_collapseText].ToString();
      }

      set
      {
        ViewState[_collapseText] = value;
      }
    }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Url of the image to use for the expand-action link."), Category("Appearance"), DefaultValue("")]
    public string ExpandImageUrl
    {
      get
      {
        ...
        return ViewState[_expandImageUrl].ToString();
      }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Text to use for the expand-action link."), Category("Appearance"), DefaultValue("Expand")]
    public string ExpandText
    {
      get
      {
        if (ViewState[_expandText] == null)
        {
          return "Expand";
        }

        return ViewState[_expandText].ToString();
      }

      set
      {
        ViewState[_expandText] = value;
      }
    }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Set to true to hide the text/image action link which" + " expands or collapses the content panel."), Category("Appearance"), 
     DefaultValue(false)]
    public bool HideActionExpandCollapse
    {
      get
      {
        if (ViewState[this._hideActionExpandCollapse] == null)
        {
          return false;
        }

        return Convert.ToBoolean(ViewState[this._hideActionExpandCollapse]);
      }

      set
      {
        ViewState[this._hideActionExpandCollapse] = value;
      }
    }
    ...
    /// value of this property between postbacks.
    /// </remarks>
    [Description("Text that will appear on the control title header."), Category("Appearance"), DefaultValue("")]
    public string TitleText
    {
      get
      {
        if (ViewState[_titleText] == null)
        {
          return string.Empty;
        }

        return ViewState[_titleText].ToString();
      }
    ...
    /// the value.
    /// </remarks>
    [Description("Display style of the control title header."), Category("Style"), DefaultValue(typeof (Style)), PersistenceMode(PersistenceMode.Attribute), 
    ...
    private void ApplyStyle(HtmlTableCell cell, Style controlStyle)
    {
      ...
      {
        cell.Style.Add("background-color", HtmlColorString(controlStyle.BackColor));
      }

      // Border colour.
      if (controlStyle.BorderColor.IsEmpty == false)
      {
        cell.Style.Add("border-color", HtmlColorString(controlStyle.BorderColor));
      }

      // Border style.
      if ((controlStyle.BorderStyle.ToString() != string.Empty) && (controlStyle.BorderStyle.ToString().ToLower() != "notset"))
      {
    ...
    {
      string titleLink = (titleIsLink == true) ? ID + "_TitleLink" : string.Empty;
      if (useImages == false)
      {
        return "javascript:DataPanel_ExpandCollapse('" + ID + "_ContentRow','" + titleLink + "','" + ID + "_ActionLink','" + ID + "_CurrentState','" +
               CollapseText + "','" + ExpandText + "');";
      }
      else
      {
        return "javascript:DataPanel_ExpandCollapseImage('" + ID + "_ContentRow','" + titleLink + "','" + ID + "_ActionLink','" + ID + "_CurrentState','" +
               ExpandImageUrl + "','" + CollapseImageUrl + "','" + CollapseText + "','" + ExpandText + "');";
      }
    }
    ...
    private void UpdateControlsState()
    {
      ...
      bool titleIsLink = AllowTitleExpandCollapse == true;

      // Are images being used for the action hyperlink?
      bool useImages = (CollapseImageUrl != string.Empty) && (ExpandImageUrl != string.Empty) && (HideActionExpandCollapse == false);

      // Sets the hidden input value to the correct collapsed state.
      this._currentState.Value = (Collapsed == true) ? "true" : "false";

      // Sets the appropriate tooltip for the title link, either
      // collapsed or expanded., if the control is collapsible.
      if ((titleIsLink == true) && (Collapsible == true))
      {
        this._titleLink.ToolTip = (Collapsed == true) ? ExpandText : CollapseText;
      }

      // Sets the title link URL.
      if ((AllowTitleExpandCollapse == true) && (Collapsible == true))
      {
        this._titleLink.NavigateUrl = PanelUrl(useImages, titleIsLink);
      }

      // Is the action link visible?
      this._actionLink.Visible = (HideActionExpandCollapse == false) ? true : false;

      // Sets the appropriate tooltip for the action link, either
      // collapsed or expanded.
      this._actionLink.ToolTip = (Collapsed == true) ? ExpandText : CollapseText;

      // Sets the appropriate tooltip for the action link, if the
      // control is collapsible.
      if (Collapsible == true)
      {
        this._actionLink.NavigateUrl = PanelUrl(useImages, titleIsLink);
        this._actionLink.Text = (Collapsed == false) ? CollapseText : ExpandText;
      }
      else
      {
      ...
      if (useImages == true)
      {
        this._actionLink.ImageUrl = (Collapsed == false) ? CollapseImageUrl : ExpandImageUrl;
      }

      // Sets the user content row element visible or invisible
      // depending on whether the control is expaned or collapsed.
      this._contentRow.Style.Add("display", (Collapsed == false) ? string.Empty : "none");
    }
    ...
    private void CreateControlComponents()
    {
      ...
      this._headerPanel.ID = HeaderPanelId;
      this._headerPanel.Visible = Visible;
      this._headerPanel.Width = Width;
      this._headerPanel.Height = Height;
      ...
      this._headerPanel.ToolTip = ToolTip;
      this._headerPanel.ApplyStyle(ControlStyle);
      ...
      this._currentState.ID = CollapsedStateId;
      this._currentState.Value = Collapsed ? "true" : "false";

      // Create the control table container.
      this._controlTable = new HtmlTable();
      this._controlTable.ID = ID + "_Table";
      ...
      ApplyStyle(this._titleCell, TitleStyle);

      // Create the title link.
      this._titleLink = new HyperLink();
      this._titleLink.ID = ID + "_TitleLink";
      this._titleLink.Text = TitleText;

      // Set the style of the link to the TitleStyle.
      this._titleLink.ApplyStyle(TitleStyle);
      this._titleLink.Visible = true;

      // Create the action cell.
      this._actionCell = new HtmlTableCell();
      this._actionCell.ID = ID + "_ActionCell";
      ...
      // Set the style of the cell to the TitleStyle.
      ApplyStyle(this._actionCell, TitleStyle);
      ...
      this._contentCell.SetRenderMethodDelegate(new RenderMethod(RenderPanelContent));
    }
    ...
    {
      string collapsedState = postCollection[CollapsedStateId];
      if (collapsedState != null)
      {
        Collapsed = (collapsedState == "false") ? false : true;
      }
