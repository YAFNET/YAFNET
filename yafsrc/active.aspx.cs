...
namespace yaf
{
	...
	public class active : BasePage
	{
			...
			if(pds.PageCount>1) {
				...
				for(int i=0;i<pds.PageCount;i++) {
					if(i==pds.CurrentPageIndex) {
						...
						PageLinks1.InnerHtml += String.Format(" <a href=\"active.aspx?p={1}&k={2}\">{0}</a>",i+1,i,SinceValue);
					}
				}
		...
		protected string GetPriorityMessage(object o) {
			DataRowView row = (DataRowView)o;
			if(row["TopicMovedID"].ToString().Length>0)
				return "[ Moved ] ";
		...
		protected string PrintForumName(DataRowView row) {
			...
			if(ForumName!=LastForumName) {
				html = String.Format("<tr><td class=header2 colspan=6><a href=\"topics.aspx?f={1}\">{0}</a></td></tr>",ForumName,row["ForumID"]);
