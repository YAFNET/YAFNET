			...
			switch (Request.QueryString["pg"])
			{
					...
					if (Request.QueryString["t"] != null) 
					{
						using(DataTable dt=DB.post_list(PageTopicID,1)) 
						{
							foreach(DataRow row in dt.Rows)
								rf.AddRSSItem(writer, row["Subject"].ToString(), Forum.GetLink(Pages.posts,"t={0}",Request.QueryString["t"]), row["Message"].ToString());
						}
					}
				...
				case "forum":
					using(DataTable dt=DB.forum_listread(PageBoardID,PageUserID,null,null)) 
					{
						foreach(DataRow row in dt.Rows) 
						{
							rf.AddRSSItem(writer, row["Forum"].ToString(), Forum.GetLink(Pages.posts,"t={0}", row["LastTopicID"]), row["Description"].ToString());
						}
					}
					break;
				case "topics":
					...
					if (Request.QueryString["f"] != null)
					{
						using(DataTable dt = DB.GetData("select Topic = a.Topic, TopicID = a.TopicID, Name = b.Name from yaf_Topic a, yaf_Forum b where a.ForumID=" + Request.QueryString["f"] + " and b.ForumID = a.ForumID")) 
						{
							foreach(DataRow row in dt.Rows)
							{
								rf.AddRSSItem(writer, row["Name"].ToString(), Forum.GetLink(Pages.posts,"t={0}", row["TopicID"]), row["Topic"].ToString());
							}
						}
					}
				...
				case "active":
					using(DataTable dt=DB.topic_active(PageBoardID,PageUserID,DateTime.Now + TimeSpan.FromHours(-24)))
					{
						foreach(DataRow row in dt.Rows)
						{
							rf.AddRSSItem(writer, row["Subject"].ToString(), Forum.GetLink(Pages.posts,"t={0}", row["LinkTopicID"]), row["Subject"].ToString());
						}
					}
					break;
				default:
