...
namespace DNA.UI.JQuery
{
  ...
  /// <summary>
  /// <para>
  /// JQueryPlugin is a WebContorl that manages the jQuery script libraries and
  ///     script files.It gernates the script for jQuery plugins just like this :
  ///     $(selector).name(options);
  /// </para>
  /// <list type="bullet">
  /// <item>
  /// Target property is <a href="http://wiki.dotnetage.com/JQueryServerSideSelector.ashx">JQuery Sever Side
  ///                                Selector</a> to apply the jQuery plugin to one or more WebControls.
  /// </item>
  /// <item>
  /// Name property use to specify the jQuery plugin name.
  /// </item>
  /// <item>
  /// The Options collection property hold all the option parameters of the
  ///       jQuery plugin options of needs.
  /// </item>
  /// <item>
  /// StartEvent property tell JQueryPlugin when jQuery scripts run
  ///       at.(ApplicationLoad,ApplicationInit,DocumentDeady)
  /// </item>
  /// </list>
  /// </summary>
  /// <remarks>
  /// <para>
  /// There are some suggestions of using jQuery plugin(s) in ASP.NET though
  ///     JQueryPlugin WebControl
  /// </para>
  /// <list type="bullet">
  /// <item>
  /// </item>
  /// <item>
  /// If the jQuery plugin has a few options i suggest to use JQueryPlugin
  ///       WebControl this is the best choice for you.
  /// </item>
  /// <item>
  /// When the jQuery plugins option has over more then 10,and the plugin has
  ...
  /// </item>
  /// <item>
  /// When the jQuery plugin options has over more than 10 and it has none ui
  ///       but it using frequently,i suggest to write the jQuery None UI WebControl by
  ///       using DNA.UI framework.
  /// </item>
  /// </list>
  /// <para>
  /// </para>
  ...
  ///   <PlugInScripts>
  ///     <Script Name="jQueryNet.ui.resizable" Assembly="jQueryNet"/>
  ///   </PlugInScripts>
  ...
  [ToolboxBitmap(typeof(JQueryPlugin), "JQueryPlugin.JQueryPlugin.ico")]
  [ParseChildren(true)]
  ...
  public class JQueryPlugin : Control
  {
    ...
    public StateManagedObjectCollection<JQueryOption> Options
    {
      get
      {
        ...
        {
          this.options = new StateManagedObjectCollection<JQueryOption>();
          if (this.EnableViewState)
          {
            ((IStateManager)this.options).TrackViewState();
          }
        }
    ...
    protected override void OnPreRender(EventArgs e)
    {
      if (!this.DesignMode)
      {
        ...
        scripts.Append(this.target.ToString(this.Page) + "." + this.name + "(");
        if (this.Options.Count > 0)
        {
          scripts.Append("{" + this.OptionsToString() + "}");
        }
    ...
    protected virtual string OptionsToString()
    {
      ...
      foreach (JQueryOption option in this.options)
      {
        if (ops.Length > 0)
        {
          ops.Append(",");
    ...
    /// <summary>
    /// The register script references.
    /// </summary>
    protected virtual void RegisterScriptReferences()
    {
      foreach (ScriptReference scriptRef in this.PlugInScripts)
      {
