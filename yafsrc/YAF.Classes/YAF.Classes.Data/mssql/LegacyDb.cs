...
namespace YAF.Classes.Data
{
	...
	public class LegacyDb
	{
			...
			bool searchDisplayName)
		{
			...
			using (var cmd = Current.GetCommand(searchSql, true))
			{
		...
		public static int? poll_save([NotNull] List<PollSaveList> pollList)
		{
			foreach (PollSaveList question in pollList)
			{
				...
				using (var cmd = Current.GetCommand(sb.ToString(), true))
				{
					...
					Current.ExecuteNonQuery(cmd, true);
		...
		public static DataTable topic_list([NotNull] object forumID, [NotNull] object userId, [NotNull] object announcement, [NotNull] object date, [NotNull] object offset, [NotNull] object count, [NotNull] object useStyledNicks, [NotNull] object showMoved, [CanBeNull]bool findLastRead)
		{
			using (var cmd = Current.GetCommand("topic_list"))
			{
				...
				cmd.AddParam("FindLastRead", findLastRead);
				return Current.GetData(cmd, true);
			}
		}
		...
		{
			using (var cmd = Current.GetCommand("user_updatefacebookstatus"))
			{
				cmd.CommandType = CommandType.StoredProcedure;
				cmd.AddParam("UserID", userID);
		...
		public static void SetProfileProperties([NotNull] int boardId, [NotNull] object appName, [NotNull] int userID, [NotNull] SettingsPropertyValueCollection values, [NotNull] List<SettingsPropertyColumn> settingsColumnsList, bool dirtyOnly)
		{
			...
			using (var conn = new MsSqlDbConnectionManager().GetOpenDbConnection())
			{
				...
				string table = DataExtensions.GetObjectName("UserProfile");
		...
		{
			string sql = @"SELECT TOP 1 * FROM {0}".FormatWith(DataExtensions.GetObjectName("UserProfile"));
		...
		public static void AddProfileColumn([NotNull] string name, SqlDbType columnType, int size)
		{
			...
			string sql = "ALTER TABLE {0} ADD [{1}] {2} NULL".FormatWith(
				DataExtensions.GetObjectName("UserProfile"), name, type);
