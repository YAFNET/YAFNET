  ...
  public class StaticDataHelper
  {
        ...
        {          
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                foreach (System.Globalization.CultureInfo ci in cultures)
                {
                    ...
                    {
                        if (tags[1, j].ToLower().Substring(0, 2).Contains(ci.TwoLetterISOLanguageName.ToLower()) && ci.IetfLanguageTag.Length == 5 && !ci.IsNeutralCulture)
                        {
                            DataRow dr = dt.NewRow();
    ...
    public static string CultureDefaultFromFile(string fileName)
    {
            ...
            foreach (System.Globalization.CultureInfo ci in cultures)
            {
                ...
                if (rawTag.ToLower().Substring(0, 2).Contains(ci.TwoLetterISOLanguageName.ToLower()) && ci.IetfLanguageTag.Length == 5 && !ci.IsNeutralCulture)
                {
