...
namespace YAF.Classes.Core
{
  ...
  public static class CreateMail
  {
    ...
    /// The message id.
    /// </param>
    public static void WatchEmail(object messageID)
    {
    ...
    /// </param>
    public static void PmNotification(int toUserID, string subject)
    {
      try
      {
        ...
        if (pmNotificationAllowed)
        {
          ...
          var pmNotification = new YafTemplateEmail("PMNOTIFICATION")
            {
          ...
          pmNotification.TemplateParams["{link}"] = String.Format(
          ...
          string emailSubject = string.Format(
            YafContext.Current.Localization.GetText("COMMON", "PM_NOTIFICATION_SUBJECT"), 
            YafContext.Current.PageUserName, 
            YafContext.Current.BoardSettings.Name, 
            subject);

          // send email
          pmNotification.SendEmail(new MailAddress(toEMail), subject, true);
        }
      }
      catch (Exception x)
      {
