...
namespace ServiceStack.FluentValidation
{
    ...
    public static class ValidatorOptions
    {
        ...
        /// </summary>
        [Obsolete(
            "Resource provider type is no longer recommended for overriding translations. Instead use the LanguageManager property.")]
        ...
        private static Func<Type, MemberInfo, LambdaExpression, string> _propertyNameResolver =
            DefaultPropertyNameResolver;

        private static Func<Type, MemberInfo, LambdaExpression, string> _displayNameResolver =
        ...
        {
            var attributes = (from attr in member.GetCustomAttributes(true) select new { attr, type = attr.GetType() })
                .ToList();

            var name = (from attr in attributes
    ...
    public class ValidatorSelectorOptions
    {
        ...
        private Func<string[], IValidatorSelector> _memberNameValidatorSelector =
            properties => new MemberNameValidatorSelector(properties);

        private Func<string[], IValidatorSelector> _rulesetValidatorSelector =
