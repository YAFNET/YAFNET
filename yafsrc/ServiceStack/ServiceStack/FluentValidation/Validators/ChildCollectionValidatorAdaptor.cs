...
namespace ServiceStack.FluentValidation.Validators
{
    ...
    public class ChildCollectionValidatorAdaptor : NoopPropertyValidator
    {
        ...
        static readonly Task<IEnumerable<ValidationFailure>> AsyncEmptyResult =
                ...
                items =>
                    {
                        ...
                        var tasks = items.Select(
                            tuple =>
                                {
                                    var (ctx, validator) = tuple;
                                    return validator.ValidateAsync(ctx, cancellation).Then(
                                        res => failures.AddRange(res.Errors),
                                        runSynchronously: true,
                                        cancellationToken: cancellation);
                                });
                        return TaskHelpers.Iterate(tasks, cancellation).Then(
                            () => failures.AsEnumerable(),
                            runSynchronously: true,
                            cancellationToken: cancellation);
                    },
                AsyncEmptyResult);
        ...
        TResult ValidateInternal<TResult>(
            ...
            TResult emptyResult)
        {
            if (!(context.PropertyValue is IEnumerable collection))
            {
                return emptyResult;
            }
            ...
            var propertyName = context.Rule.PropertyName;
            ...
            var itemsToValidate = collection.Cast<object>().Select((item, index) => new { item, index })
                .Where(a => a.item != null && predicate(a.item)).Select(
                    a =>
                        {
                            var newContext = context.ParentContext.CloneForChildValidator(a.item);
                            newContext.PropertyChain.Add(propertyName);
                            newContext.PropertyChain.AddIndexer(
                                a.item is IIndexedCollectionItem
                                    ? ((IIndexedCollectionItem)a.item).Index
                                    : a.index.ToString());
        ...
        {
            if (!(expression.Body is ParameterExpression paramExp))
            {
                throw new InvalidOperationException(
                    "Could not infer property name for expression: " + expression
                                                                     + ". Please explicitly specify a property name by calling OverridePropertyName as part of the rule chain. Eg: RuleFor(x => x).SetCollectionValidator(new MyFooValidator()).OverridePropertyName(\"MyProperty\")");
            }
