...
public class SqlServer2016Expression<T> : SqlServerExpression<T>
{
    ...
    private object BooleanJsonExpression(string predicate, string scalar)
    {
        ...
        return new PartialSqlString(scalar);
    }

    protected override object VisitIsJsonMethod(object json)
    {
        var scalar = this.DialectProvider is SqlServer2022OrmLiteDialectProvider
            ? $"ISJSON({json}, VALUE)"
            : $"ISJSON({json})";
        return this.BooleanJsonExpression($"({scalar} = 1)", scalar);
    }
    ...
    protected override object VisitJsonValueMethod(object json, JsonPathExpression path, Type returnType)
    {
        ...
        if (type == typeof(string) || type.IsEnum)
            return this.JsonScalar(value, returnType);

        if (type == typeof(bool))
        {
            var boolean = $"CASE {value} WHEN 'true' THEN CAST(1 AS bit) WHEN 'false' THEN CAST(0 AS bit) END";
            return this.JsonScalar(boolean, returnType);
        }

        return this.JsonScalar($"TRY_CONVERT({this.GetJsonDbType(type)}, {value})", returnType);
    }
    ...
    {
        if (this.DialectProvider is not SqlServer2022OrmLiteDialectProvider)
    ...
    {
        var typeCode = this.JsonTypeCode(json, path);
    ...
    private string JsonTypeCode(object json, JsonPathExpression path)
    {
        ...
        var parent = this.QuoteJsonPath(parentPath);
        var quotedKey = this.DialectProvider.GetQuotedValue(key, typeof(string));
    ...
    protected override object VisitJsonArrayContainsMethod(object json, JsonPathExpression path, object value, Type valueType)
    {
        ...
        {
            comparison = $"j.[type] = 2 AND TRY_CONVERT({this.GetJsonDbType(valueType)}, j.[value]) = {value}";
        }
        ...
        return this.BooleanJsonExpression(predicate, scalar);
    }
}