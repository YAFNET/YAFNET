...
namespace ServiceStack.Templates
{
    ...
    public partial class TemplateDefaultFilters
    {
        ...
        public string spaces(int count) => padLeft(string.Empty, count, ' ');
        ...
        public IRawString formatRaw(object obj, string fmt) =>
            raw(string.Format(fmt.Replace("{{", "{").Replace("}}", "}"), obj));
        ...
        public string timeFormat(TimeSpan timeValue) =>
            timeValue.ToString((string)Context.Args[TemplateConstants.DefaultTimeFormat]);
        ...
        public string substringWithElipsis(string text, int startIndex, int length) =>
            text.SubstringWithElipsis(startIndex, length);
        ...
        public int indexOf(string text, string needle) =>
            text.IndexOf(needle, (StringComparison)Context.Args[TemplateConstants.DefaultStringComparison]);

        public int indexOf(string text, string needle, int startIndex) =>
            text.IndexOf(needle, startIndex, (StringComparison)Context.Args[TemplateConstants.DefaultStringComparison]);

        public int lastIndexOf(string text, string needle) =>
            text.LastIndexOf(needle, (StringComparison)Context.Args[TemplateConstants.DefaultStringComparison]);

        public int lastIndexOf(string text, string needle, int startIndex) =>
            text.LastIndexOf(
                needle,
                startIndex,
                (StringComparison)Context.Args[TemplateConstants.DefaultStringComparison]);

        public int compareTo(string text, string other) =>
            string.Compare(text, other, (StringComparison)Context.Args[TemplateConstants.DefaultStringComparison]);
        ...
        public string padLeft(string text, int totalWidth, char padChar) =>
            text?.PadLeft(AssertWithinMaxQuota(totalWidth), padChar);
        ...
        public string padRight(string text, int totalWidth, char padChar) =>
            text?.PadRight(AssertWithinMaxQuota(totalWidth), padChar);
        ...
        public string[] split(string stringList, object delimiter)
        {
            ...
            if (delimiter is IEnumerable<string> strDelims)
                return strDelims.All(x => x.Length == 1)
                           ? stringList.Split(
                               strDelims.Select(x => x[0]).ToArray(),
        ...
        public Dictionary<string, string> parseKeyValueText(string target, string delimiter) =>
            target?.ParseKeyValueText(delimiter);
        ...
        public string addQueryString(string url, object urlParams) =>
            urlParams.AssertOptions(nameof(addQueryString)).Aggregate(
                url,
                (current, entry) => current.AddQueryParam(entry.Key, entry.Value));

        public string addHashParams(string url, object urlParams) =>
            urlParams.AssertOptions(nameof(addHashParams)).Aggregate(
                url,
                (current, entry) => current.AddHashParam(entry.Key, entry.Value));
        ...
        public IRawString jsString(string text) =>
            string.IsNullOrEmpty(text) ? RawString.Empty : escapeNewLines(escapeSingleQuotes(text)).ToRawString();

        [HandleUnknownValue]
        public IRawString jsQuotedString(string text) =>
            ("'" + escapeNewLines(escapeSingleQuotes(text ?? string.Empty)) + "'").ToRawString();
        ...
        public IRawString jsv(object value) => serialize(value, null, x => x.ToJsv() ?? string.Empty);

        public IRawString jsv(object value, string jsconfig) =>
            serialize(value, jsconfig, x => x.ToJsv() ?? string.Empty);

        public IRawString csv(object value) => (value.AssertNoCircularDeps().ToCsv() ?? string.Empty).ToRawString();

        public IRawString dump(object value) => serialize(value, null, x => x.Dump() ?? string.Empty);

        // Blocks
        public Task json(TemplateScopeContext scope, object items) => json(scope, items, null);

        public Task json(TemplateScopeContext scope, object items, string jsConfig) =>
            serialize(scope, items, jsConfig, x => x.ToJson());

        public Task jsv(TemplateScopeContext scope, object items) => jsv(scope, items, null);

        public Task jsv(TemplateScopeContext scope, object items, string jsConfig) =>
            serialize(scope, items, jsConfig, x => x.ToJsv());

        public Task dump(TemplateScopeContext scope, object items) => jsv(scope, items, null);

        public Task dump(TemplateScopeContext scope, object items, string jsConfig) =>
            serialize(scope, items, jsConfig, x => x.Dump());
        ...
        public Dictionary<string, object> jsonToObjectDictionary(string json) =>
            json.FromJson<Dictionary<string, object>>();
        ...
        public Dictionary<string, object> jsvToObjectDictionary(string json) =>
            json.FromJsv<Dictionary<string, object>>();

        public Dictionary<string, string> jsvToStringDictionary(string json) =>
            json.FromJsv<Dictionary<string, string>>();
