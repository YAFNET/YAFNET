...
namespace YAF.Web
{
    ...
    public class UserBox : BaseControl
    {
        ...
        /// </returns>
        [NotNull]
        private static string RemoveEmptyDividers([NotNull] string userBox)
        {
            userBox = userBox.Replace("<li class=\"list-group-item\">{0}</li>", string.Empty);
                    ...
                    role =>
                        {
                            ...
                            foreach (var drow in roleStyleTable.Rows.Cast<DataRow>().Where(
                                drow => drow["LegendID"].ToType<int>() == 1 && drow["Style"] != null
        ...
        private string MatchUserBoxLocation([NotNull] string userBox)
        {
            ...
            if (this.UserProfile.Location.IsSet())
            {
                filler = string.Format(
                    ...
                    this.Get<IFormatMessage>().RepairHtml(this.HtmlEncode(this.UserProfile.Location), false));
            }
        ...
        private string MatchUserBoxMedals([NotNull] string userBox)
        {
            ...
            if (this.Get<YafBoardSettings>().ShowMedals)
            {
                ...
                // vzrus: If user doesn't have we shouldn't render this wasting resources
                if (!dt.HasRows())
                {
        ...
        {
            var rankStyle = (from DataRow dataRow in roleStyleTable.Rows
            ...
            var filler = string.Format(
                ...
                this.Get<YafBoardSettings>().UseStyledNicks
                    ? string.Format(
                        @"<span class=""YafRank_{0}"" style=""{1}"">{0}</span>",
        ...
        private string MatchUserBoxCountryImages([NotNull] string userBox)
        {
            ...
            if (this.Get<YafBoardSettings>().ShowCountryInfoInUserBox && this.UserProfile.Country.IsSet()
                                                                      && !this.UserProfile.Country.Equals("N/A"))
            {
        ...
        private string MatchUserBoxThanksFrom([NotNull] string userBox)
        {
            ...
            {
                filler = string.Format(
                    this.Get<YafBoardSettings>().UserBoxThanksFrom,
                    string.Format(
                        this.UserProfile.Gender == 1 ? this.GetText("thanksfrom_musc") :
                        this.UserProfile.Gender == 2 ? this.GetText("thanksfrom_fem") : this.GetText("thanksfrom"),
        ...
        private string MatchUserBoxThanksTo([NotNull] string userBox)
        {
            ...
            if (this.Get<YafBoardSettings>().EnableThanksMod)
            {
                ...
                {
                    filler = string.Format(
                        this.Get<YafBoardSettings>().UserBoxThanksTo,
                        string.Format(
                            this.GetText("thanksto"),
