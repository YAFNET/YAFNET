...
[ApiController]
[AdminAuthorization]
public class BannedUserAgentController : ForumBaseController
{
    /// <summary>
    /// Import
    /// </summary>
    ...
    public async Task<IActionResult> Import([FromForm] IFormFile file)
    {
        ...
        try
        {
            var importedCount = await this.Get<IDataImporter>().BannedUserAgentsImportAsync(
            ...
            return this.Ok(
                new MessageModalNotification(
                 importedCount > 0
                    ? string.Format(this.GetText("ADMIN_BANNED_USERAGENTS_IMPORT", "IMPORT_SUCESS"), importedCount)
        ...
        catch (Exception x)
        {
            this.Get<ILogger<BannedEmailController>>().Error(
                x,
                string.Format(this.GetText("ADMIN_BANNED_USERAGENTS_IMPORT", "IMPORT_FAILED"), x.Message));
    ...
    public IActionResult Edit([FromBody] BannedEmailEditModal model)
    {
        ...
        if (!this.GetRepository<BannedUserAgent>().Save(
                model.Id,
                model.Mask.Trim()))
        {
            return this.Ok(
