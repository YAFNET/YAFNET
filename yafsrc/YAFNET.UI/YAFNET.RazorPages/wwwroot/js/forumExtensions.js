...
function getSearchResultsData(pageNumber) {
    ...
    var searchForum = document.getElementById("Input_ForumListSelected").value === "" ? 0 : parseInt(document.getElementById("Input_ForumListSelected").value);
    var searchText = "";
...
document.addEventListener("DOMContentLoaded", function() {
    if (document.querySelector(".searchSimilarTopics") != null) {
        ...
        input.addEventListener("keyup", () => {
            const placeHolder = document.getElementById("SearchResultsPlaceholder"), ajaxUrl = "/api/Search/GetSimilarTitles", searchText = input.value;
...
function startChat() {
    ...
    chatHub.on("onConnected", function(toUserId, conversations) {
        for (let i = 0; i < conversations.length; i++) {
            const ctrId = `private_${toUserId}`, control = document.getElementById(ctrId), avatar = control.dataset.avatar;
    ...
    chatHub.on("sendPrivateMessage", function(toUserId, message, toAvatar, dateTime) {
        const ctrId = `private_${toUserId}`, currentUserId = parseInt(document.getElementById("UserId").value);
        var side = "start";
...
(() => {
    ...
    const showActiveTheme = theme => {
        ...
        const themeSwitcherText = document.getElementById("bd-theme-text"), activeThemeIcon = document.querySelector(".theme-icon-active"), btnToActive = document.querySelector(`[data-bs-theme-value="${theme}"]`), iconOfActiveBtn = btnToActive.querySelector("i").className, checkOfActiveBtn = btnToActive.querySelector(".fa-check");
