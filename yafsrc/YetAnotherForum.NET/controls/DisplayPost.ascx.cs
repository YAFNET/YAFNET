	...
	public partial class DisplayPost : BaseUserControl
	{
		...
		protected string FormatThanksInfo([NotNull] string rawStr)
		{
			...
			foreach (var chunk in rawStr.Split(','))
			{
				...
				{
					sb.AppendFormat(
						@" {0}", this.GetText("DEFAULT", "ONDATE").FormatWith(this.Get<IDateTime>().FormatDateShort(thanksDate)));
				}
			}
		...
		private void DisplayPost_PreRender([NotNull] object sender, [NotNull] EventArgs e)
		{
			...
			if (suspended.HasValue && suspended.Value > DateTime.UtcNow)
			{
				this.ThemeImgSuspended.LocalizedTitle =
					this.GetText("POSTS", "USERSUSPENDED").FormatWith(this.Get<IDateTime>().FormatDateTimeShort(suspended.Value));
			...
			this.MovePost.NavigateUrl = YafBuildLink.GetLinkNotEscaped(ForumPages.movemessage, "m={0}", this.PostData.MessageId);
			...
			YafContext.Current.PageElements.RegisterJsBlockStartup("syntaxhighlighterjs", JavaScriptBlocks.SyntaxHighlightLoadJs);
			...
			switch (e.Item)
			{
				...
				case "lastposts":
					...
					YafBuildLink.Redirect(
						ForumPages.search, "postedby={0}", displayName.IsNotSet() ? this.PostData.UserProfile.UserName : displayName);
					break;
				case "addbuddy":
					...
					{
						this.PageContext.AddLoadMessage(this.GetText("NOTIFICATION_BUDDYAPPROVED_MUTUAL").FormatWith(strBuddyRequest[0]));
				...
				case "removebuddy":
					{
						...
						this.PageContext.AddLoadMessage(
							this.GetText("REMOVEBUDDY_NOTIFICATION").FormatWith(this.Get<IBuddy>().Remove(this.PostData.UserId)));
						break;
					}
