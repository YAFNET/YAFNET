...
<br />
<p><em><asp:Label ID="albumsInfo" Visible="false" runat="server"></asp:Label></em></p>
<div>
                    ...
                    <tr>
                        <td class="albumtitlebox">
                            <span runat="server" id="spnUser" visible='<%# UserID != PageContext.PageUserID %>'>
                                <%# this.HtmlEncode(Eval("Title"))%></span> <span runat="server" id="spnAlbumOwner" visible='<%# UserID == PageContext.PageUserID %>'>
                                    <span class="albumtitle" id='<%# "spnTitle0" + Eval("AlbumID") %>' onclick="showTexBox(this.id)"
                                        style="display: inline;"><%# Eval("Title").ToString() == string.Empty ? PageContext.Localization.GetText("ALBUM_CHANGE_TITLE") : this.HtmlEncode(Eval("Title"))%></span>
