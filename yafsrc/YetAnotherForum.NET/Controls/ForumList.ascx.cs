    ...
    public partial class ForumList : BaseUserControl
    {
        ...
        public string GetForumLink([NotNull] ForumRead item)
        {
            ...
            if (item.ReadAccess)
            {
                ...
                output = item.RemoteURL.IsSet()
                             ...
                             : $"<a href=\"{BuildLink.GetForumLink(forumID, output)}\" data-bs-toggle=\"tooltip\" title=\"{title}\">{this.Page.HtmlEncode(output)}</a>";
            }
            else
            {
        ...
        protected void ForumList1_ItemCreated([NotNull] object sender, [NotNull] RepeaterItemEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    forumImage.ImageUrl =
                        $"{BoardInfo.ForumClientFileRoot}{BoardFolders.Current.Forums}/{item.ImageURL}";
