	...
	var bbcodeHandlers = {
		...
		albumimg: {
			...
			html: function (token, attrs, content) {
				...
				return '<div class="card text-bg-dark" style="max-width:200px" alt="' + content + '">' +
					'<img src="/api/Albums/GetImagePreview?imageId=' + escapeUriScheme(attrs.defaultattr) +
					'" class="img-user-posted card-img-top" style="max-height:200px;max-width:200px;object-fit:contain"">' +
					'</div>';
			}
		},
		// END_COMMAND
		...
		quote: {
			...
			html: function (token, attrs, content) {
				if (attrs.defaultattr) {
					content = content +
						'<cite class="card-text text-end d-block"><small class="text-body-secondary">' +
						escapeEntities(attrs.defaultattr) +
						'</small></cite>';
				}

				return '<div class="card mx-3 mb-3 border-secondary shadow-sm">' +
					'<div class="card-body"><p class="card-text">' +
					'<i class="fa fa-quote-left text-primary fs-4 p-2"></i><span class="quote-content">' +
					content +
					'</span></p></div></div>';
			}
		},
		...
		code: {
			...
			format: function (element, content) {
				...
				return '[code=' + codeLanguage.replace('language-', '') + ']' +
                    content + '[/code]';
			},
