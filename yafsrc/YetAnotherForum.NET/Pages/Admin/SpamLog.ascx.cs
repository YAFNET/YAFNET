...
public partial class SpamLog : AdminPage
{
    ...
    protected string FormatStackTrace(string input)
    {
        try
        {
            ...
            try
            {
                var addressLink = this.HtmlEncode(string.Format(this.PageBoardContext.BoardSettings.IPInfoPageURL, json.UserIP));

                var exceptionSource = ((string)json.ExceptionSource).IsSet() ?
                    $"""<span class="badge text-light-emphasis bg-light-subtle m-1"><i class="fa-solid fa-code me-1"></i>{this.HtmlEncode((string)json.ExceptionSource)}</span>"""
                    : "";

                var url = ((string)json.Url).IsSet()
                              ? $"""<span class="badge text-bg-secondary m-1"><i class="fa-solid fa-globe me-1"></i>{(string)json.Url}</span>"""
                              : "";
                ...
                var userAgent = ((string)json.UserAgent).IsSet()
