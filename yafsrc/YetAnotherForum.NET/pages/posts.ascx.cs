				...
				else
				{
					...
					if (this.Get<HttpRequestBase>().QueryString.GetFirstOrDefault("find") == null)
					{
						if (this.Get<HttpRequestBase>().QueryString.GetFirstOrDefault("m") != null)
						{
							...
							using (DataTable lastPost = LegacyDb.message_findunread(topicID: this.PageContext.PageTopicID, messageId: this.Get<HttpRequestBase>().QueryString.GetFirstOrDefault("m"), lastRead: DateTime.MinValue.AddYears(1902), showDeleted: showDeleted, authorUserID: userId))
							{
					...
					else
					{
						...
						{
							DateTime lastRead = this.Get<IReadTrackCurrentUser>().GetForumTopicRead(forumId: this.PageContext.PageForumID, topicId: this.PageContext.PageTopicID);

							// Find next unread
							using (
									DataTable unread = LegacyDb.message_findunread(topicID: this.PageContext.PageTopicID, messageId: 0, lastRead: lastRead, showDeleted: showDeleted, authorUserID: userId))
							{
						...
						if (this.Get<HttpRequestBase>().QueryString.GetFirstOrDefault("find").ToLower() == "lastpost")
						{
							// Find next unread
							using (
									DataTable unread = LegacyDb.message_findunread(topicID: this.PageContext.PageTopicID, messageId: 0, lastRead: DateTime.UtcNow, showDeleted: showDeleted, authorUserID: userId))
							{
								var unreadFirst = unread.AsEnumerable().FirstOrDefault();
								if (unreadFirst != null)
								{
