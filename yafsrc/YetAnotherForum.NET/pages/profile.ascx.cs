...
namespace YAF.Pages
{
    ...
    public partial class profile : ForumPage
    {
        ...
        private void LocalizeTabTitles(int userID)
        {
            ...
            this.ProfileTabs.Views["BuddyListTab"].Text = this.GetText(userID == this.PageContext.PageUserID ? "BUDDIES" : "BUDDIESTITLE");
          ...
          int userID, MembershipUser user, CombinedUserDataHelper userData, string userDisplayName)
        {
            ...
            {
                this.LastVisit.Text = this.Get<YafDateTime>().FormatDateTime(userData.LastVisit);
            }
            ...
            if (this.User != null && userData.Profile.Birthday != DateTime.MinValue)
            {
                ...
                this.Birthday.Text =
                  this.Get<YafDateTime>().FormatDateLong(userData.Profile.Birthday.Date.Add(-this.Get<YafDateTime>().TimeOffset));
            }
            else
            {
