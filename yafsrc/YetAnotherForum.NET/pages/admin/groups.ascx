  ...
  <table class="content" width="100%" cellspacing="1" cellpadding="0">
    <asp:Repeater ID="RoleListNet" runat="server" OnItemCommand="RoleListNet_ItemCommand">
			<HeaderTemplate>
				...
				<tr>
					<td colspan="6"  class="post">Note: Deleting a role here 
					removes it completely from the provider. "Add to YAF" to make this role accessible in the forum.</td>
				</tr>	
				<tr>
					<td class="header2" colspan="5">Name</td>
					<td class="header2">&nbsp;</td>				
				</tr>
			</HeaderTemplate>
			<ItemTemplate>
				<tr>
					<td class="post" colspan="5"><%# Container.DataItem %> (Unlinked)</td>
          <td class="post">
            ...
            |
            <asp:LinkButton ID="LinkButtonDelete" runat="server" OnLoad="Delete_Load" CommandName="delete" CommandArgument='<%# Container.DataItem %>'>Delete Role</asp:LinkButton>
          </td>
    ...
    <asp:Repeater ID="RoleListYaf" runat="server" OnItemCommand="RoleListYaf_ItemCommand">
      ...
      <ItemTemplate>
        <tr>
          <td class="post">
            <%# Eval( "Name" ) %> (<%# GetLinkedStatus( (System.Data.DataRowView) Container.DataItem )%>)
          </td>
          <td class="post">
            <%# BitSet(Eval( "Flags" ),2) %>
          </td>          
          ...
          <td class="post">
            ...
            |
            <asp:LinkButton ID="LinkButtonDelete" runat="server" OnLoad="Delete_Load" Visible='<%#(BitSet(Eval( "Flags" ),2) == true ? false : true)%>' CommandName="delete" CommandArgument='<%# Eval( "GroupID") %>'>Delete from YAF</asp:LinkButton>
          </td>
        </tr>
      </ItemTemplate>
    </asp:Repeater>
