...
namespace YAF.Pages
{
    ...
    public partial class search : ForumPage
    {
        ...
        {          
            if (!this.IsPostBack)
            {
                ...
                if (YafServices.Permissions.Check(this.PageContext.BoardSettings.ExternalSearchPermissions))
                {
                    ...
                    if (!string.IsNullOrEmpty(this.PageContext.BoardSettings.SearchEngine1) && (!string.IsNullOrEmpty(this.PageContext.BoardSettings.SearchEngine1Parameters)))
                    {
                        this.btnSearchExt1.Visible = true;
                        this.btnSearchExt1.Text = this.btnSearchExt1.ToolTip = this.PageContext.Localization.GetText("btnsearch") + " " + PageContext.BoardSettings.SearchEngine1Parameters.Split('^')[0];
                    }
               
                    if (!string.IsNullOrEmpty(this.PageContext.BoardSettings.SearchEngine2) && (!string.IsNullOrEmpty(this.PageContext.BoardSettings.SearchEngine2Parameters)))
                    {
                        this.btnSearchExt2.Visible = true;
                        this.btnSearchExt2.Text = this.btnSearchExt2.ToolTip = this.PageContext.Localization.GetText("btnsearch") + " " + PageContext.BoardSettings.SearchEngine2Parameters.Split('^')[0];
                    }
                }              
        ...
        /// </param>
        /// <returns>
        /// An HtmlForm by ControlCollection value
        /// </returns>
        ...
        /// <param name="e">The e - EventArgs</param>
        ...
        /// </summary>
        /// <param name="i">A Command option.</param>
        /// <returns>An external search link with already inserted searchparameters.</returns>
        ...
        /// A search string.
        /// </summary>
        /// <param name="searchEngine">A search engine template string.</param>
        /// <param name="searchParams">A parameters template string</param>
        /// <returns>Returns a ready search engine string.</returns>
        private string TransformExtSearchLink(string searchEngine, string searchParams)
        {
            searchEngine = searchEngine.Replace("{ResultsPerPage}", this.listResInPage.SelectedValue.ToString());
        ...
        /// </summary>
        /// <param name="searchParamsOptArrayj">A parameter string</param>
        /// <param name="searchEngine">An output Search engine String</param>
        /// <param name="searchParamsDefArraySep">A search word separator</param>
        private void MatchParameterHelper(ref string searchParamsOptArrayj, ref string searchEngine, string searchParamsDefArraySep)
        {
            searchParamsOptArrayj = searchParamsOptArrayj.Replace("{Word}", this.txtSearchStringWhat.Text + (!string.IsNullOrEmpty(this.txtSearchStringFromWho.Text) ? " " + this.txtSearchStringFromWho.Text : string.Empty));
            
            // Add "+"  between words
            searchParamsOptArrayj = searchParamsOptArrayj.Replace(" ", searchParamsDefArraySep);
            searchParamsOptArrayj = searchParamsOptArrayj.Replace(searchParamsDefArraySep + searchParamsDefArraySep, searchParamsDefArraySep);
