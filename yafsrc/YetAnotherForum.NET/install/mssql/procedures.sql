...
begin
	...
	IF @ShowMoved = 1
		SELECT
			...
			LinkTopicID = IsNull(c.TopicMovedID,c.TopicID),
			c.TopicMovedID,
			FavoriteCount = (SELECT COUNT(1) as [FavoriteCount] FROM [{databaseOwner}].[{objectQualifier}FavoriteTopic] WHERE TopicId = c.TopicMovedID OR (c.TopicMovedID IS NULL AND TopicId = c.TopicID)),
			[Subject] = c.Topic,
			c.UserID,
			Starter = IsNull(c.UserName,b.Name),
