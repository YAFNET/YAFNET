...
namespace YAF.Modules
{
	...
	public class LastVisitEventHandler : IHandleEvent<ForumPagePreLoadEvent>, IHandleEvent<ForumPageUnloadEvent>
  {
    ...
    /// The yaf session.
    /// </param>
    public LastVisitEventHandler([NotNull] IYafSession yafSession, HttpRequestBase requestBase, HttpResponseBase responseBase)
    {
    ...
    public void Handle([NotNull] ForumPagePreLoadEvent @event)
    {
    	...
    	if (!YafContext.Current.IsGuest && YafContext.Current.Page[previousVisitKey] != DBNull.Value && !this.YafSession.LastVisit.HasValue)
      {
